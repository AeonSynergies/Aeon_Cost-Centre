...
import { ServiceAddModal, PackageModal, type PackageEditing } from "@/components/modals/ServiceModals";
...
type ServiceRow = {
  id: string; code: string; name: string; departmentId: string;
...
export default function ServicesPage() {
  ...
  return (
    <PageShell
      ...
      actions={
        tab === "services"
          ? <Button onClick={() => setSvcOpen(true)}><Plus size={14} /> Add Service</Button>
    ...
    >
      <Tabs value={tab} onValueChange={setTab} className="flex min-h-0 flex-1 flex-col">
        ...
        <TabsContent value="services" className="flex min-h-0 flex-1 flex-col">
          <DataTable columns={serviceCols} data={services} loading={isLoading} onRowClick={(r) => router.push(`/services/${r.id}`)}
            empty={{ icon: <Layers size={32} />, heading: "No services", cta: <Button onClick={() => setSvcOpen(true)}><Plus size={14} /> Add Service</Button> }} />
        </TabsContent>
      ...
      <ServiceAddModal open={svcOpen} onOpenChange={setSvcOpen} onSaved={() => mutate()} />
